...
int64_t Token::toNumber() const 
{
  ...
  if (kind == INT) 
  {
    iss.str(lexeme);  //convert string to integer
  } 
  else if (kind == HEXINT) 
  {
    iss.str(lexeme.substr(2));    //convert part after 0x to an integer
    iss >> std::hex;
  } 
  else if (kind == REG) 
  {
...
const std::string &ScanningFailure::what() const 
{ 
  return message; 
}
...
std::vector<Token> scan(const std::string &input) //O(n), where n is the # of tokens on the line
{
  static AsmDFA theDFA;
  std::vector<Token> tokens = theDFA.simplifiedMaximalMunch(input); //O(m), where m = # characters in a line
